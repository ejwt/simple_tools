...
void main()
{
  ...
  printf("This program is used to split a binary file. There are two modes.\nMode 0: Split the file evenly \
...
input_again1:
  printf("\nEnable large file mode (can process file >= 2GiB, but slower) 0: no, 1: yes : ");
  scanf("%u", &large_en);
...
here3:
  ...
  if (split_mode == 0)
  {
    ...
    INT32U  dst_block_size_MiB = 1; // for source file >= 2GiB
...
input_again4:
        ...
        {
          printf("\nError! dst_block_size must belong to [1, %u]. Please input again.\n", src_length);
          goto input_again4;
        }
        ...
        {
          printf("\nError! The number of destination blocks is more than %u, please input a smaller block size.\n", MAX_NUM_DST_BLOCK);
...
the file into more than %u pieces.\n", MAX_NUM_DST_BLOCK);
        ...
        {
          printf("\nThe last destination block is %u bytes. (Smaller than other blocks)", last_block_size);
          extra_dst_block_flag = 1; // total number of destination blocks is (num_dst_block + 1)
        }
        printf("\nThe file will be splitted into %u blocks. Double click join.bat can join them together.", (num_dst_block + extra_dst_block_flag));
        }
        else  /* large file (>=2GiB) */
        {
input_again4L0:
        printf("\nInput the size (in MiB) of the destination block: ");
        scanf("%u", &dst_block_size_MiB);
        if ( dst_block_size_MiB < 1 )
        {
          printf("\nError! dst_block_size must be larger than 1 MiB. Please input again.\n");
          goto input_again4L0;
        }
      ...
      if (NULL == (fp_dst_block = fopen(fname_dst_block_all, "wb"))) //create destination block
      {
        ...
        goto Exit_prog;
          ...
          for (j=0; j<dst_block_size; j++) // copy data
          {
            ...
            {
          for (j=0; j<dst_block_size_MiB; j++) // copy data
          {
                    if ( feof(fp_src) )
                    {
            ...
            fwrite(MiB_buffer, 1, MiB, fp_dst_block);
          }
            }
